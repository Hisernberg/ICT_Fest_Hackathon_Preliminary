...
# recorded here and any further use is rejected.
...
def consume_refresh_token(payload: dict) -> None:
    """Mark a presented refresh token as used; reject any reuse."""
    jti = payload.get("jti")
    with _refresh_lock:
        if jti is None or jti in _used_refresh_tokens:
            raise AppError(401, "UNAUTHORIZED", "Refresh token no longer valid")
